...
target_dir="$root_dir/apps/web/src/features/editor-v2/components"
...
shopt -s nullglob
for file_path in "$target_dir"/*.tsx; do
  ...
  story_path="$target_dir/$base_name.stories.tsx"
  ...
  (
    ...
    pnpm exec hygen story new \
      --name "$base_name" \
      --dir "apps/web/src/features/editor-v2/components" \
      --exportStyle "named" \
      --exportName "$export_name" \
      --titlePrefix "Features/EditorV2"
  )
done
