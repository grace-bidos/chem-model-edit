...
**Independent Test**: 小スラブ出力の最終座標から変位を計算し、大スラブの可動原子に適用した .in が生成されることを確認できる。

**Acceptance Scenarios**:

1. **Given** 小スラブの .in/.out と大スラブの .in が揃っている、**When** Δ移植を実行、**Then** 可動フラグのある原子にのみ変位が適用された .in が出力される
2. **Given** 小/大で可動原子数が一致しない、**When** Δ移植を実行、**Then** 実行は中断され、理由が提示される
...
- **FR-005b**: システムは小スラブ入力・小スラブ出力・大スラブ入力を受け取り、可動原子の緩和変位を大スラブへΔ移植できる
...
- **SC-003b**: Δ移植で可動原子の変位が小スラブ出力の最終座標に一致する
