...
| Queue target ownership and selection behavior (Convex-backed runtime target store) | Tier-1 | Storage-backed runtime target behavior can be validated with store stubs and contract tests | `uv run pytest apps/api/tests/test_runtime_targets.py` |
